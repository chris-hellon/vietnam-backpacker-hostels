...
namespace VietnamBackpackerHostels.UI.Pages.AboutUs.CityGuides
{
    public class IndexModel : BasePageModel
    {
        ...
        public async Task<IActionResult> OnGet()
        {
            try
            {
                var page = await OverridePageId("city-guides");
                ...
                if (_pageId.HasValue)
                {
                    ...
                    if (pageSections.Any())
                    {
                        ...
                        if (Categories != null)
                            Category = Categories.PageSectionContent.Where(s => s.NavigatePageCategory == "city-guides").FirstOrDefault();

                        CityGuides = pageSections.Where(ps => ps.Id != 28);
                    }
                }
